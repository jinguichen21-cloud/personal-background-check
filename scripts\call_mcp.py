...
URL 支持两种格式：
    ...
    2. 环境变量名： "$MCP_BOCHA_URL"（从系统环境或 mcp-config.json 读取）
...
MCP Server URL 获取方式：
    ...
    4. 写入技能目录下的 mcp-config.json，格式：{"MCP_BOCHA_URL": "https://..."}
"""
...
def load_mcp_config():
    """从技能目录查找并加载 mcp-config.json，回退到 .env 兼容模式。

    查找优先级：技能根目录 > scripts 目录 > 当前工作目录
    系统环境变量优先级最高，配置文件不会覆盖已有的系统变量。
    """
    search_dirs = [
    ...
    # 优先读 mcp-config.json（非隐藏文件，不受运行态安全管控）
    for directory in search_dirs:
        config_file = directory / "mcp-config.json"
        if config_file.exists():
            try:
                with open(config_file, encoding="utf-8") as f:
                    config = json.load(f)
                for key, value in config.items():
                    # 新格式：{"MCP_FOO": {"url": "...", "mcpId": "..."}}
                    if isinstance(value, dict):
                        url = value.get("url", "")
                        if url and key not in os.environ:
                            os.environ[key] = url
                    # 兼容旧格式：{"MCP_FOO": "https://..."}
                    elif isinstance(value, str) and value and key not in os.environ:
                        os.environ[key] = value
                return str(config_file)
            except (json.JSONDecodeError, OSError) as e:
                print(f"⚠️  读取 mcp-config.json 失败：{e}", file=sys.stderr)

    # 降级兼容：读取 .env 文件（部分运行态可能因安全管控无法访问）
    for directory in search_dirs:
                ...
                with open(env_file, encoding="utf-8") as f:
                    for line in f:
                        ...
                        if key and key not in os.environ:
                            os.environ[key] = value
                return str(env_file) + " (兼容模式，建议迁移到 mcp-config.json)"
            except OSError:
                pass
...
def resolve_url(url_or_var):
    ...
    if url_or_var.startswith("$"):
        ...
        if not value:
            ...
            print(f"   请在技能目录的 mcp-config.json 中添加：\"{var_name}\": \"<your_url>\"", file=sys.stderr)
...
def main():
    # 启动时加载 mcp-config.json（或降级 .env）
    env_file = load_mcp_config()
